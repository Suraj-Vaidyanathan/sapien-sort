...
const PackageTable: React.FC<PackageTableProps> = ({ packages }) => {
  ...
  return (
    <Card className="p-3 h-full">
      <h3 className="text-base font-semibold text-gray-800 mb-2">Package Information</h3>
          ...
          <tbody>
            {packages.map(pkg => (
              <tr key={pkg.uid} className="border-b hover:bg-gray-50">
                ...
                <td className="py-2 px-2">
                  <span className={`px-1 py-0.5 rounded-full text-[10px] font-medium ${getStatusColor(pkg.status)}`}>
                    {pkg.status}
                  </span>
                </td>
